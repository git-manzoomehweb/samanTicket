...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    if (innerWidth < 1024) {
        document.querySelectorAll(".image-category-magamenu , .sub-item-megamenu").forEach(element => {
            element.remove();
        })
    } else {
        // document.querySelectorAll(".sub-item-hamburgermenu").forEach(element => {
        ...
        document.querySelectorAll(".megamenu-container").forEach(ele => {
          ...
          console.log(ele)
            ...
            const catid = ele.querySelector(".magamenu-category").getAttribute('data-catid');
            const typecat = ele.querySelector(".magamenu-category").getAttribute('data-typecat');
...
async function LoadCatHeader(catid, typecat, element) {
    ...
    try {
        ...
        if (window.innerWidth >= 1024) {
            element.closest(".megamenu-container").querySelector(".load-category-items").innerHTML = content;
...
function OpenSubMenu() { this.querySelector("ul").classList.toggle("hidden") }
document.addEventListener("DOMContentLoaded", function () {
    let submenuparents = document.querySelectorAll(".has-sub-menu");
    if (window.innerWidth < 1024) { submenuparents.forEach((e => { e.addEventListener("click", OpenSubMenu, false) })); }
});
